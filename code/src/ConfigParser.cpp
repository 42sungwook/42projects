...
RootBlock *ConfigParser::addBlock(RootBlock *block, enum BLOCK type)
{
  ...
  else
  {
    ...
    if (_locationMap.find(static_cast<ServerBlock *>(block)) == _locationMap.end())
      _locationMap[static_cast<ServerBlock *>(block)] = new LocationList;
    _locationMap[static_cast<ServerBlock *>(block)]->push_back(static_cast<LocationBlock *>(newBlock));
  }
  return newBlock;
}
